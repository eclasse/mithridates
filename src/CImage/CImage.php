...
class CImage {
    ...
    public function __construct($directory, $src) {
        
        $this->dir = $directory;
        $this->src = $src;
        $this->maxHeight = $this->maxWidth = 2000;
    ...
    private function validateParams()
    {
        ...
        substr_compare(strtolower($this->imagePath), strtolower($this->pathToImage), 0, strlen($this->imagePath)) == 0 or $this->errorMessage('Security constraint: Source image is not directly below the directory IMG_PATH.');
...
EOD;
        ...
        else if($this->newWidth && !$this->newHeight) 
        {
            ...
            if($this->verbose) 
            { 
                $this->verbose("New width is now {$this->newWidth}, height is calculated to {$this->newHeight}."); 
            }
        }
        else if(!$this->newWidth && $this->newHeight) 
        {
            ...
            if($this->verbose) 
            { 
                $this->verbose("New height is now {$this->newHeight}, width is calculated to {$this->newWidth}."); 
            }
        }
        else if($this->newWidth && $this->newHeight) 
        {
        ...
        */
        ...
        $cacheFileName = $this->cachePath . "-{$dirName}-{$parts['filename']}_{$this->newWidth}_{$this->newHeight}{$quality_}{$cropToFit_}{$sharpen_}.{$this->saveAs}";
